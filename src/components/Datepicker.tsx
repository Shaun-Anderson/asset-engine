        ...
        <Reference>
          {({ ref }) => (
            <div className={`relative flex ${className}`} ref={ref}>
              <input
                className="border rounded-lg text-sm dark:text-white px-2 py-3 outline-none dark:border-zinc-800 flex-grow dark:bg-transparent"
                type="text"
                // style={inputStyle}
              ...
              <button
                type="button"
                className="absolute inset-y-0 right-0 pr-3 rounded-r flex items-center justify-center text-sm font-semibold text-gray-700 dark:text-white px-2 focus:outline-none"
                onClick={(e) => ctxValue.toggleCalendar()}
              >
                <CalendarIcon size="18" stroke={1} />
              </button>
            </div>
          )}
        </Reference>
...
>((props, ref) => {
  ...
  return (
    <div
      className="bg-white dark:bg-gray-900 relative shadow-lg max-w-xs w-64 p-2 rounded-lg z-20"
      ref={ref}
...
const DateSelection: React.FC<{}> = (props) => {
  ...
  for (let i = 1; i <= daysInMonth(month, year); i++) {
    dates.push(
      <button
        key={`day${i}`}
        className={`hover:bg-gray-200 dark:hover:bg-gray-600 rounded p-2 text-sm ${
  ...
  return (
    ...
    >
      <button
        ...
        onClick={(e) => prevMonth()}
      >
        <IconChevronLeft size={20} className="stroke-current" />
      </button>
      ...
      <button
        ...
        onClick={(e) => nextMonth()}
      >
        <IconChevronRight size={20} className="stroke-current" />
      </button>
...
const MonthSelection: React.FC<{}> = (props) => {
  ...
  return (
    <div
      className="h-48 text-gray-800 dark:text-gray-500"
      style={{
        display: "grid",
...
const buttonClassName =
  "hover:bg-gray-200 dark:hover:bg-gray-600 rounded p-2 text-sm flex align-center justify-center focus:outline-none items-center";
...
const CalButton: React.FC<{
  ...
  if (props.chevron && props.chevron === "left")
    children = <IconChevronLeft size={20} className="stroke-current" />;
  else if (props.chevron && props.chevron === "right")
    children = <IconChevronRight size={20} className="stroke-current" />;
